    ...
    try {
      ...
      if ((decompressed_size != expected_decompressed_size) && (decompressed_size != expected_decompressed_size - 8)) {
        static_game_data_log.warning_f(
            "decompressed size ({}) does not match expected size ({}); ignoring check", decompressed_size, expected_decompressed_size);
      }
    } catch (const std::exception& e) {
      static_game_data_log.warning_f(
          "failed to check decompressed size ({}); ignoring check", e.what());
    }
